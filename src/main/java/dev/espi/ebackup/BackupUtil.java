...
public class BackupUtil {
    ...
    private static void checkMaxBackupsFTP(FTPClient ftpClient) {
        
        if (eBackup.getPlugin().maxBackups <= 0) return;
        
        int backups = 0;
        SortedMap<Long, FTPFile> m = new TreeMap<>(); // oldest files to newest
        
        try {
            for (FTPFile f : ftpClient.listFiles()) {
                if (f.getName().endsWith(".zip")) {
                    backups++;
                    m.put(f.getTimestamp().getTimeInMillis(), f);
                }
            }
            
            // delete old backups
            while (backups-- >= eBackup.getPlugin().maxBackups) {
                FTPFile ftpFile = m.get(m.firstKey());
